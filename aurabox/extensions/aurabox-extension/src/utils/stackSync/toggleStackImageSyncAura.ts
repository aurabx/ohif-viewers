...
const STACK_SYNC_NAME = 'stackImageSyncAura';
...
export default function toggleStackImageSyncAura({
  ...
  const viewports =
    providedViewports ||
    getReconstructableStackViewports(viewportGridService, displaySetService);
...
function disableSync(syncName, servicesManager) {
  ...
  const viewports = getReconstructableStackViewports(
    viewportGridService,
    displaySetService
  );
  viewports.forEach((gridViewport) => {
